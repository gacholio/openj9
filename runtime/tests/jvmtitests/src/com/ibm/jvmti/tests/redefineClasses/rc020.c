...
static void
showMethods(JNIEnv *jni_env, jclass clazz, char *name)
{
	...
	err = (*jvmti_env)->GetClassMethods(jvmti_env, clazz, &count, &methods);
	if (err != JVMTI_ERROR_NONE) {
	...
	for (jint i = 0; i < count; ++i) {
		char *name = NULL;
		char *sig = NULL;
		err = (*jvmti_env)->GetMethodName(jvmti_env, methods[i], &name, &sig, NULL);
		if (err != JVMTI_ERROR_NONE) {
                        error(env, err, "Unable to allocate temp buffer for the class file");
		} else {
			printf("\t%s%s\n", name, sig);
