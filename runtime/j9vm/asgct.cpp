...
ticks_thread_exit           = -8, // dying thread or otherwise invalid JNIEnv
...
protectedASGCT(J9PortLibrary *portLib, void *arg)
{
	...
	if (NULL == targetThread) {
		parms->num_frames = ticks_thread_exit;
	} else {
...
void AsyncGetCallTrace(ASGCT_CallTrace *trace, jint depth, void *ucontext)
{
	...
	if (NULL != BFUjavaVM) {
		...
		if (J9_EVENT_IS_HOOKED(BFUjavaVM->hookInterface, J9HOOK_VM_CLASS_LOAD)) {
			...
			num_frames = ticks_no_class_load;
		}
	}
#endif /* ASGCT_SUPPORTED */
